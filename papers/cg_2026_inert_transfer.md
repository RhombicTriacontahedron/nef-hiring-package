# The Inert Transfer: Redistribution in an Economy That Produces Its Own Workforce
...
$$N_{j,t+1} = g_{j,t}\,N_{j,t}, \qquad j\in\{W,C\}, \tag{$\star$}$$

where $g_{j,t}$ is class $j$’s growth factor, the reproduction technology of Assumption 2 below.
Write $\kappa_W \equiv (1-\alpha)+(1-\phi)\tau\alpha$ for labour’s share of output plus the cash
...
rates satisfy $\delta,\delta_G \in (0,1]$. All three instruments are shares: $\tau_t \in [0,\bar\tau]$
with $\bar\tau<1$, $\phi_t \in [0,1]$, $i_t \in [0,1]$.

**Assumption 2 (reproduction, with a common elasticity).** $B_W, B_C>0$, $\gamma \in (0,1)$,
$\theta \in (0,1]$, and

$$g_{W,t} = B_W\big(c_{W,t}^{\theta}\,z_t^{1-\theta}\big)^{\gamma}, \qquad
  g_{C,t} = B_C\,c_{C,t}^{\gamma}, \qquad z_t \equiv G_t/N_{W,t}. \tag{1}$$

The two classes share the **elasticity** $\gamma$ and may differ arbitrarily in the **level** $B_j$.
Each map $u\mapsto B_j u^{\gamma}$ is strictly increasing and strictly concave. The public input enters
the worker’s reproduction and the owner’s not at all.

Reproduction here is a *technology* carrying this period’s resources into next period’s numbers, not a
fertility choice. $\gamma$ is the elasticity of that technology, and separating it from $B_j$ is what
makes the asymmetry question answerable rather than rhetorical. Suppose the two classes differ in the
resources they command, in the medicine those resources buy, and in how efficiently what they spend
becomes surviving numbers. Every one of those is a difference in $B_j$: it shifts the map up or down,
and Assumption 2 permits it without restriction. To differ in $\gamma$ they would have to differ in the
...
$N_W$, no price for it exists, and no instrument in $(\star)$ transfers a claim on it. That absence is
not a modelling convenience. It is the reason §5’s conclusion is available at all.
...
$$x_C \;=\; \chi\,x_W, \qquad \chi \;\equiv\; \Big(\frac{B_W}{B_C}\Big)^{1/\gamma}, \tag{2}$$
...
**Proposition 1 (cash is inert).** *Let Assumptions 1–2 hold and impose $\theta=1$, so that a worker is
reproduced by private consumption alone. Then:*

*(i) the public stock leaves both transition equations identically and the $(x,\nu)$ block closes, so
the economy reduces to a two-state system;*

*(ii) at every interior stationary class ratio the per-head gap is the constant of Lemma 1,*

$$\frac{c_C}{c_W} \;=\; \chi \;=\; \Big(\frac{B_W}{B_C}\Big)^{1/\gamma}, \tag{3}$$

*in which the tax rate $\tau$, the spending composition $\phi$, the reinvestment rate $i$, the
productivity level $A$, the capital share $\alpha$, both depreciation rates and the class ratio itself
are all absent, as is every property of either class’s objective;*

*(iii) the class ratio at such a state is*
...
*(a) the per-head gap is a constant, independent of the level at which the economy sits, if and only if
$\gamma_W=\gamma_C$; when it is, that constant is $\chi$ of (3), and $\chi=1$ if and only if
$B_W=B_C$;*
...
the first will be scored as a failure by an instrument that only reads the second.

# 4. The instrument that is not inert

Switch the public input back on. Now the worker’s reproduction runs on a composite of private
consumption and public provision, the two classes’ *indices* still equalise by Lemma 1, and what fails
to equalise is exactly what the state has prised apart.

**Theorem 1 (stationary incidence).** *Let Assumptions 1–2 hold with $\theta\in(0,1)$, and let
$(x,\nu,z)$ be any interior stationary class ratio. Then the per-head gap is a closed form in the
...
$$\frac{\partial \ln c_W}{\partial \ln z}\Big|_{\,\chi\,c_W^{\theta}z^{1-\theta}\,=\,c_C}
  \;=\; -\frac{1-\theta}{\theta}\;<\;0; \tag{8}$$

*the two private claims are ordered by a single comparison,*
...
fixed by the technology and not by the budget — raise provision by a tenth and the private claim
compatible with the same reproduction falls by $(1-\theta)/\theta$ tenths, at every level of provision
...
the split, the share of output the worker keeps after it, the public stock’s own depreciation, and $g$.

*The reproduction levels enter only as a multiplicative constant.* $\chi$ scales (10) and appears
nowhere in (8). The asymmetry that §3 showed the cash instrument cannot touch is also an asymmetry
...
stationary class ratios generated by the admissible instrument set
$(\tau,\phi,i)\in[0,\bar\tau]\times[0,1]^2$. Then*
...
instruments can generate. The second is that those states exist for every admissible instrument vector
...
before. In this economy the factor whose per-head return is at issue is the workforce, and the
