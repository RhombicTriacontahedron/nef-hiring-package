...
What does not survive is the reduction. Assign a labour magnitude to a unit of the good and the familiar series closes — the labour done directly, plus the labour in the capital worn out making it, and so on down — exactly where replacement takes less than the product. Extend the same assignment to the making of the worker and it fails, and the failure is not a residual of measurement. It is exact, it is bounded below by the owners’ own two uses of the product, and it can be made to vanish only inside a window in the growth factor that is never wider than the capital share. Two properties one wants such a magnitude to have turn out to be available only on thin sets of the primitives, and they are different sets. It is **additive** over the worker’s bundle if and only if the public stock has left his continuation altogether. It is **the same at two consecutive dates** if and only if the rest point lies in that window. Off the window the books still close, and they close by giving one physical good two different magnitudes at two dates at which the technique, every per-worker quantity and the good itself are identical — a magnitude of the date rather than of the commodity, which is not what a labour magnitude was wanted for.
...
Two things follow that the valuation route does not deliver. First, the rate exists whether or not anyone wants it: dating the magnitudes always solves, so the question is never whether an intertemporal ratio can be had, only whether it can be **one**. Second, and this is the constraint the accounting places on the anchor rather than a use of it — a magnitude cannot be both consistent and a property of the commodity except on a thin set. Off the window of Proposition 8 the books close only by giving one good two magnitudes at two dates at which the technique, every per-worker quantity and the good itself are identical. The reproduction process does supply a rate. The rate it supplies is not unity, and the set on which it is unity is never wider than the capital share.
