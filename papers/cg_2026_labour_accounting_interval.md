...
The question behind that statistic is older than it. *Capital* treats the wage bill as one historical form of appearance of the fund a worker must himself produce and reproduce, and holds the maintenance and reproduction of the working class to be a necessary condition of the reproduction of capital (ch. 23). Both survive here intact, and identity (R) below is the second of them written as an equation: what a worker’s period yields beyond the cost of maintaining him is owner consumption plus the net enlargement of the two stocks, and no rule may charge either of those to him. A stock funded from a tax on profits is then another form of appearance of the same fund, so calling it a claim on capital states an incidence inside this instrument set. It says nothing about whose labour produced the output the claim is drawn from, and nothing below settles that.

What does not survive is the reduction. Assign a labour magnitude to a unit of the good and the familiar series closes — the labour done directly, plus the labour in the capital worn out making it, and so on down — exactly where replacement takes less than the product. Extend the same assignment to the making of the worker and it fails, and the failure is not a residual of measurement. It is exact, it is bounded below by the owners’ own two uses of the product, and it can be made to vanish only inside a window in the growth factor that is strictly narrower than the capital share. Two properties one wants such a magnitude to have then fail, and they fail differently. It is never **additive** over the worker’s bundle: no total with positive weights on both inputs ranks continuation bundles as the index does, at any $\theta$, and agreement requires dropping the public stock from the total altogether. And it is **the same at two consecutive dates** only when the growth factor lies in that window *and* the charge is the one that closes — a joint condition on the configuration and the rule, not a property of the commodity. Off the window the books still close, and they close by giving one physical good two different magnitudes at two dates at which the technique, every per-worker quantity and the good itself are identical.
...
both positive and strictly decreasing on the open range, because $\alpha<1$. Hence $w(g)=Ax(g)^{\alpha}$ and $c_W(g)=\kappa_W w(g)$ are strictly decreasing, and by (1) so is $c_C(g)=c_W(g)^{\theta}z(g)^{1-\theta}$, which is strictly positive throughout. The stationary condition $g=Bc_C(g)^{\gamma}$ then fixes the scale directly: $B=g\,c_C(g)^{-\gamma}$, positive and unique at every admissible target, with no limiting argument needed. Equivalently $\ln B=\mathcal{H}(g)$, where
...
**Proposition 5 (the index admits no additive total).** *Let $S=p_c c_W+p_z z$ with $p_c,p_z>0$. Then for **every** $\theta\in(0,1]$, $S$ and $c_W^{\theta}z^{1-\theta}$ induce distinct ordinal rankings on an open set of bundles in $\mathbb{R}_{++}^{2}$. Coincidence requires $p_z=0$, which is the degenerate total that has already dropped the public stock.*

*Proof.* For $\theta\in(0,1)$, level sets of the index satisfy $z=k\,c_W^{-\theta/(1-\theta)}$ for $k>0$, and

$$
\frac{d^{2}z}{dc_W^{2}}=\frac{\theta}{(1-\theta)^{2}}\,k\,c_W^{-\theta/(1-\theta)-2}>0 ,
$$

so they are strictly convex, while level sets of $S$ are lines; a line cannot coincide with a strictly convex curve on a nonempty open set. At $\theta=1$ the index is $c_W$, which ties every pair sharing a first coordinate, while $p_z>0$ makes $S$ rank them: at $p_c=p_z=1$ the bundles $(1,1)$ and $(1,2)$ are tied by the index and ranked by $S$. Hence the rankings differ at $\theta=1$ as well. If instead $p_z=0$, then $S$ is a positive multiple of $c_W$ and agrees with the index exactly at $\theta=1$. $\square$

A published total of wages plus benefits is an instance of $S$. It does not rank continuation bundles, and no choice of prices makes it do so while the public stock is still an input. Proposition 5 is an impossibility and carries no positive counterpart: what a long-run configuration does hold, once no additive magnitude represents the index, is a different question and is not pursued here.
...
**Proposition 6 (production conserves, and the condition is stronger than interiority).** *Production conserves if and only if $\chi(w-\delta x)=1$. A strictly positive pair $(\varpi,\chi)$ exists if and only if $\zeta<1$, and is then unique up to a common scale; with the numéraire fixed at $\varpi=1$, $\chi$ itself is unique, and*
...
*an interval of length $\alpha[\tau+\delta i(1-\tau)]<\alpha$, strictly, at every interior rest point, since $\tau<1$ and $\delta i<1$ there. Writing $u=g-1$ and*

$$
Q(u):=\frac{u\bigl(u+\delta(1-\iota)\bigr)}{u+\delta},
$$

*$Q$ is strictly increasing on $u\ge0$ with $Q(0)=0$ and $Q(u)\to\infty$, so for every admissible tuple with $\phi\tau>0$ the band is a **nonempty** subinterval $[g_-,g_+]$ of $(1,2)$, its edges the unique positive roots of $Q(u)=\kappa_W$ and $Q(u)=\kappa_W+\varsigma$; by Proposition 3 every point of it is the growth factor of an interior rest point, for a unique $B$. No interior rest point with $g\le1$ closes under a rule that charges at least $c_W$. No interior rest point with $g>2$ closes under a rule that charges at most net output; at $g=2$ exactly, closure occurs, and only by charging the entire net product $b=w-\delta x$. Under a closing rule inside $\mathcal{B}$, $R=(2-g)(w-\delta x)>0$.*
...
The length of the necessary interval is $\alpha[\tau+\delta i(1-\tau)]$, and the gap to $\alpha$ is $\alpha(1-\tau)(1-\delta i)$, strictly positive at every interior rest point because $\tau<1$ and $i<1$ there. The interval is therefore strictly shorter than the capital share, and approaches it only as a fully depreciating private stock is wholly reinvested — a limit interiority does not contain.

*Exact edges, and the band is never empty.* Substituting $\zeta=\delta\iota/m$ and $m=u+\delta$ into the middle member of (7) gives $Q(u)$, with $Q(0)=0$ and $Q(u)\to\infty$; and

$$
Q'(u)=\frac{(u+\delta)^{2}-\delta^{2}\iota}{(u+\delta)^{2}}>0
$$

on $u\ge0$ because $\iota<1$. The equation $Q(u)=\kappa$ clears to $u^{2}+u[\delta(1-\iota)-\kappa]-\kappa\delta=0$, whose roots have product $-\kappa\delta<0$; exactly one is positive. Since $\kappa_W\ge1-\alpha>0$ and $\kappa_W<\kappa_W+\varsigma$ whenever $\varsigma>0$, both roots exist and are ordered, so the band is a nonempty interval; it lies in $(1,2)$ by the two edges just derived.

*Remainder.* Proposition 7 gives $R=(2-g)(w-\delta x)$, and (8) forces $g<2$ inside $\mathcal{B}$ while Proposition 6 forces $w-\delta x>0$, so $R>0$ there. Separately, a closing rule of any kind satisfies $R\ge0$, because a period cannot be charged more than it yields; with $w-\delta x>0$ that forces $g\le2$ with no reference to $\mathcal{B}$ at all. The bound is attained: at $g=2$ the closing bundle is $b^{\ast}=(g-1)(w-\delta x)=w-\delta x$, the whole net product, and $R=0$. So $g>2$ is excluded outright and $g=2$ is excluded only by $\mathcal{B}$, which caps the charge at $c_W+\varsigma w<w-\delta x$. $\square$
...
The window is never empty, and it is reached. Emptiness is not the obstacle and never was: for every admissible tuple with $\phi\tau>0$ the band is a genuine interval of growth factors at which some rule in $\mathcal{B}$ closes the accounting, and Proposition 3 puts an interior rest point at each of them. What fails is occupancy, not existence. At $\alpha=3/4$ with $\tau=\phi=i=\delta=\delta_G=1/2$, the band (7) is $[7/16,\,5/8]$, and at $g=3/2$ the replacement share is $\zeta=3/32$ and the closing share is $(g-1)(1-\zeta)=29/64$, strictly inside it, so a rule in $\mathcal{B}$ closes the system there. The upper edge binds just as concretely. At $\alpha=\tau=i=\delta=\delta_G=1/2$ with $\phi=1$, the point $g=3$ satisfies the lower edge and violates the upper, the closing bundle being $1.95\,w$ against a maximum defensible charge of $0.75\,w$. Both are exact rational arithmetic in the instruments and the growth factor, which is all the two edges of (7) contain.

Neither configuration is merely stated. A growth factor is not free, so by Proposition 3 each is *induced*: for any $A>0$, $\theta\in(0,1]$ and $\gamma\in(0,1)$ one adds to the instruments above, there is exactly one $B=g\,c_C(g)^{-\gamma}>0$ putting a rest point at that $g$, with every instrument held. The scale $B$ is not recoverable from the instruments alone — it carries $A$, $\theta$ and $\gamma$ — so no numerical value for it is quoted here.
...
using $\kappa_W+\varsigma+\iota+s_C=1$, $\zeta=\delta\iota/m$, and $\iota w=\delta x+(g-1)x$. Subtracting the two endpoints gives the width. Since $\kappa_W+\varsigma=(1-\alpha)+\tau\alpha$ does not carry $\phi$, write $\bar\kappa$ for it; then the width is $(1-\zeta)\varsigma/[(\bar\kappa-\varsigma)\bar\kappa]$, and $d[\varsigma/(\bar\kappa-\varsigma)]/d\varsigma=\bar\kappa/(\bar\kappa-\varsigma)^{2}>0$. It is zero if and only if $\varsigma=0$, since $1-\zeta>0$ and $\kappa_W>0$.
...
The floor $\varepsilon_-$ is net private accumulation plus owner consumption, measured against the most the class may charge a worker. Those are precisely the two uses of the period’s product that no rule may charge to him, so nothing in the floor cancels: it could vanish only if both vanished, and they vanish together only at $g=1$, which kills accumulation, with $i=1$, which kills owner consumption — a pair that is not in the domain. The floor reaches the split only through the growth factor. Using $x/w=\iota/m$, the floor is $\varepsilon_-=\bigl[(g-1)\iota/m+s_C\bigr]/\bar\kappa$, and none of $\iota$, $s_C$ and $\bar\kappa$ carries $\phi$; so with the growth factor held the split does not move it at all, while along the family of Theorem 1, where the growth factor is reselected, it moves by that one route alone. The route is strictly increasing, $\partial[(g-1)/m]/\partial g=\delta/m^{2}>0$, so the floor’s direction along the family is the direction of $\partial g/\partial\phi$, which carries the sign of $P$ and takes either. The floor rises with the split wherever (14) fails, rests at its equality, and falls where it holds — the one edge whose reversal asks nothing beyond (14), since the width must clear (15) as well, reaching the split by three channels where the floor reaches it by one. The ceiling and the width move directly as well. Two normalisations are in play and they answer different questions: $R/b$ makes the closing condition an interval that either does or does not contain a point, which is what Propositions 8 and 9 need, while $R/w$ makes the floor a share of output, which is what names its composition.
...
$$
\varepsilon_+-\varepsilon_-=\bigl(1+\varepsilon_+\bigr)\,\frac{\overline{b}-\underline{b}}{\overline{b}} .
\tag{10}
$$

*This uses no property whatever of the economy that produced $N$, $\underline{b}$ and $\overline{b}$. (ii) In this model $\underline{b}=c_W$, $\overline{b}=c_W+\varsigma w$ and $N=w-\delta x$, so that*

$$
\frac{\overline{b}-\underline{b}}{\overline{b}}=\frac{\varsigma}{\kappa_W+\varsigma}=\frac{\phi\tau\alpha}{(1-\alpha)+\tau\alpha},
\tag{11}
$$

*which carries no growth factor, no replacement share and no scale constant $A$, and is strictly increasing in the split and in the tax.*
...
$$
\frac{\partial g_+}{\partial\phi}=0,
\qquad
\frac{\partial g_-}{\partial\phi}<0,
\qquad
\frac{\partial\,(g_+-g_-)}{\partial\phi}>0 ,
\tag{12}
$$

*so the band of growth factors at which the accounting can be closed at all widens with the split, entirely by lowering its floor. As $\phi\downarrow0$ the two edges meet — an endpoint Proposition 2 removes from the domain.*
...
It is worth saying what $\rho$ is, because it is a ratio of a commodity’s magnitude at one date to its magnitude at the next, and objects of that shape have a home. The proposition that the value of wealth and the rate of interest have a material anchor in the reproduction of labour, rather than a merely empirical one, is not established here and is not claimed here; a research programme exists that derives a discount rate from a sustenance problem instead of importing it from asset markets, and the anchor is that programme’s, not this paper’s.

What is this paper’s is the **route**, and the route changes what the object can be asked to do. A discount rate obtained from a sustenance problem is a **valuation** object: something is being priced, and an intertemporal trade-off is being solved. $\rho$ is not that. Nobody in this model prices anything, no agent chooses across dates, no preference and no kernel appears, and there is no risk to adjust for. $\rho$ is a **consistency** object — the rate at which a labour magnitude must move for a labour-only charge to balance two activities that share a good. Given the rule, it is fixed by conservation and nothing else.

Two things follow that the valuation route does not deliver. First, the rate exists whether or not anyone wants it: dating the magnitudes always solves, so the question is never whether an inter-temporal ratio can be had, only whether it can be **one**.

Second — and this is the constraint the accounting places on the anchor rather than a use of it — the constraint has to be quantified carefully, because two quantifiers are easy to fuse here. Fix the instruments and the depreciation rates. Then *some* rule in $\mathcal{B}$ makes the magnitude date-invariant exactly on the band (7), whose necessary container is strictly shorter than the capital share; and for a rule *already selected*, $\rho=1$ exactly when $b=b^{\ast}$. What does **not** follow is a rule-independent magnitude. Since $\partial\rho/\partial b=1/(g\,n)>0$, the rate moves with the charge even inside the band, so date-invariance is a property of a configuration together with a rule, and never of the commodity on its own. The reproduction process does supply a rate; it does not supply one number, and the rules that make it unity are confined to a band no economy occupies.

The rate is therefore an interval as well, and it is the third object here whose width is the public investment share. At a rest point $\rho=(n+b)/(g\,n)$ with $n=w-\delta x$, so as $b$ runs through $\mathcal{B}$,
...
The cancellation is exact, and it is worth stating exactly, because the slogan available here is stronger than the algebra. Raising the split feeds the public stock directly, at rate $a$; the stationary system answers by moving the growth factor, which moves the stock’s own investment margin the other way; and what $\gamma P$ removes from $\gamma(1-\theta)(a+b)$ is exactly $\gamma(1-\theta)a$ — the whole of the direct channel’s contribution **inside the continuation component**. It does not remove $a$ from the numerator, because the two stationary terms carry $(a+b)$ and are positive. So $\mathcal{N}$ is positive for two independent reasons: the worker’s falling cash share, isolated as $\gamma b$ by the cancellation, and the stationary response, which would sign the derivative on its own. What the cancellation establishes is that the direct provision channel is not among those reasons. A dominance argument would have been weaker and a different result.
...
The coextension of production-conservation and reproduction-conservation is an artefact of a one-input subsistence bundle. Once continuation uses a public stock built from profits, a labour-only ledger that balances goods does not balance people, except at growth factors confined to a bounded band. The defect is identity (R). Every rule in $\mathcal{B}$ relocates the social wage inside that identity, and none deletes the right-hand side while owners consume and the two stocks grow.
...
Closure inside $\mathcal{B}$ is a window in the growth factor, strictly narrower than the capital share at every interior rest point, empty at a yearly reading of ordinary calibrations, and not a property of any particular set of national accounts. Faster growth is not an escape: the same laws induce rest points that breach the upper edge, and one is exhibited above.
...
What travels is the unfunded hole, identity (R), the impossibility of an additive representation, the necessary window, the interval $\varepsilon=R/b$, the ratio (11) that takes the growth feedback out of the latitude, the one-sided widening (12) of the window under the split, the no-drift reading of closure, and the cancellation that signs the private gap. These do not travel equally far, and the distance divides them three ways. Identity (10) uses nothing about the economy at all: it is a fact about charge classes. Proposition 5 uses the form of the continuation index and nothing else — no rest point, no policy, no law of motion enters its proof — so it holds in any economy whose continuation runs on the two inputs in that form, and the published total it refuses is refused there too. Every other item is a fact about this one. They are statements about a rest point of stated laws under a stated charge, inside the constant-share class, and every step of Sections 4 and 5 is a statement about a single date. Existence of such a configuration under a wider class of policies is not shown here, and neither is uniqueness. This is not a theory of value, not a net social wage for any country, and not an equilibrium of the game whose laws have been used.
...
-   Huato, J., and C. Galindo Escajeda. “Labor Power as Value Source and the Form of Capitalist Wealth.”
    Working paper.
